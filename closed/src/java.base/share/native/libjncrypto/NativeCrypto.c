...
static void * find_crypto_library(jboolean traceEnabled, jboolean skipBundledLib, const char *chomepath);
 ...
 */
static void *
find_crypto_library(jboolean traceEnabled, jboolean skipBundledLib, const char *chomepath)
{
    ...
    /* Look in JAVA_HOME for a bundled library if the user hasn't requested skipping it. */
    if (JNI_TRUE == skipBundledLib) {
        if (traceEnabled) {
...
Java_jdk_crypto_jniprovider_NativeCrypto_loadCrypto
  (JNIEnv * env, jclass clazz, jboolean traceEnabled, jboolean skipBundledLib, jstring jlibname, jstring jhomepath)
{
    const char *chomepath = "";
    jlong ossl_ver = 0;

    if (JNI_FALSE == skipBundledLib) {
        if (NULL != jlibname) {
     ...
     */
    if (NULL == crypto_library) {
        crypto_library = find_crypto_library(traceEnabled, skipBundledLib, chomepath);
    }
